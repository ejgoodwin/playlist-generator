...
class RelatedTracks extends HTMLElement {
  ...
  getRelatedTracks_() {
    ...
    const limit = Math.floor(
      this.relatedArtistsIds.length / this.artistIds.length,
    );
  ...
  displayTracks_(track) {
    ...
    // Add slot info.
    trackElement.insertAdjacentHTML(
      "beforeEnd",
      `<span slot="track-title">${track.name}</span>`,
    );
    trackElement.insertAdjacentHTML(
      "beforeEnd",
      `<span slot="track-artist">${track.artists[0].name}</span>`,
    );
    trackElement.insertAdjacentHTML(
      "beforeEnd",
      `<span slot="track-album">${track.album.name}</span>`,
    );
    trackElement.insertAdjacentHTML(
      "beforeEnd",
      `<span slot="track-time">${duration}</span>`,
    );
    trackElement.insertAdjacentHTML(
      "beforeEnd",
      `<img slot="track-image" alt="" class="track__image" src=${track.album.images[0].url}>`,
    );
    ...
    if (trackListItems.length > 1) {
      ...
      while (
        trackListItems[randomTrackInsert].hasAttribute("selected") &&
        randomTrackInsert != 0
      ) {
        randomTrackInsert--;
      }
